...
Use the title `<RoleCode>|<Model> [<Reasoning>]|<Route>|<Platform>`. Platform is the execution surface that owns the session; route is the provider or backend supplying the model. Examples are `E|GLM 5.2 [High]|Ollama|OpenCode`, `E|Composer 2.5|Cursor|Cursor`, `R|Opus 5 [High]|Anthropic|Claude Code`, and `E|SWE-2 Max|Devin-managed|Devin Local`. Store it with `acp start --title`; use a filesystem-safe slug of the title plus the project name as the session `--name`.

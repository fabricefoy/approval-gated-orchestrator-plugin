...
Use the title `<RoleCode>|<Model> [<Reasoning>]|<Route>|<Platform>`. Platform is the execution surface that owns the session; route is the provider or backend supplying the model. Examples are `E|GLM 5.2 [High]|Ollama|OpenCode`, `E|Composer 2.5|Cursor|Cursor`, `R|Opus 5 [High]|Anthropic|Claude Code`, and `E|SWE-2 Max|Devin-managed|Devin Local`. Store it with `acp start --title`; use a filesystem-safe slug of the title plus the project name as the session `--name`. Devin and OpenCode also display the title in their own session lists, and Cursor keeps it in the bridge only. For subagents, use the title as the `Agent` tool's `description`. If an approved change alters an executor's identity, for example its model, rename it with `acp title` so that the title and the live `model` field agree.

Match on the session's live `model` from `acp list --json`, not on the title text alone. Continue a running exact match with another `acp prompt`, or resume a stopped one with `acp start <agent> --name <id> --resume`. Never create a duplicate. If the match is busy, wait for it rather than creating another. If it cannot be resumed (the agent lacks `session/load`, or the load fails), report that and obtain owner approval before creating a replacement.
